...
# --- BLOCO DE INICIALIZAÇÃO E EXTRAÇÃO DOS ASSETS ---

# 1. Cria um diretório temporário para guardar nossos arquivos extraídos
ASSETS_DIR = tempfile.mkdtemp()

# 2. Define uma função para limpar (apagar) o diretório temporário quando o app fechar
def cleanup():
    print(f"Limpando diretório temporário: {ASSETS_DIR}")
    shutil.rmtree(ASSETS_DIR)
atexit.register(cleanup)

# 3. A função resource_path agora encontra o assets.zip
...
# 4. Encontra e extrai o assets.zip para o diretório temporário
assets_zip_path = resource_path('assets.zip')
if os.path.exists(assets_zip_path):
    with zipfile.ZipFile(assets_zip_path, 'r') as zip_ref:
        zip_ref.extractall(ASSETS_DIR)
else:
    # Fallback para modo de desenvolvimento (se assets.zip não existir, usa os arquivos locais)
    ASSETS_DIR = os.path.abspath(".")


# --- CONFIGURAÇÃO DO APP FLASK ---
# Agora, o Flask vai usar as pastas DENTRO do nosso diretório temporário
...
# --- FUNÇÕES AUXILIARES ---
# Todas as funções que abrem arquivos agora vão procurar dentro do diretório temporário
def carregar_config():
...
def carregar_perguntas(nome_arquivo):
    """ Lê um arquivo de perguntas específico do diretório de assets. """
    perguntas_path = os.path.join(ASSETS_DIR, nome_arquivo)
    # ... (o resto da função carregar_perguntas continua o mesmo, apenas usando perguntas_path)
    perguntas = []
    try:
        with open(perguntas_path, 'r', encoding='utf-8') as f:
            blocos_de_perguntas = f.read().strip().split('---')
            # ... (resto do seu código de parse) ...
            for bloco in blocos_de_perguntas:
                ...
                perguntas.append({
                    'pergunta': texto_pergunta, 'opcoes': textos_das_opcoes,
                    'resposta': resposta_correta, 'explicacao': texto_explicacao
                })
    except FileNotFoundError:
        print(f"AVISO: O arquivo '{perguntas_path}' não foi encontrado.")
        return []
    return perguntas

def carregar_placar(modo):
    """Lê o placar para um modo específico."""
    # Placares ainda são salvos localmente, não no zip, para persistirem
...
def salvar_placar(placar, modo):
    """Salva o placar para um modo específico."""
    nome_arquivo = f"placar_{modo}.json"
