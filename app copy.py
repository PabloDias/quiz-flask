...
## MUDANÇA: A definição da função precisa vir ANTES de ser usada.
...
## MUDANÇA: Agora usamos a função para definir os caminhos das pastas.
...
def carregar_config():
    ...
    try:
        ## MUDANÇA: Usando resource_path para encontrar o config.json
    ...
    try:
        with open(resource_path(nome_arquivo), 'r', encoding='utf-8') as f:
            ...
            for bloco in blocos_de_perguntas:
                ...
                # NOVO: Procura pela linha de explicação
                texto_explicacao = None
                # Filtra a explicação das outras linhas
                ...
                perguntas.append({
                    ...
                    'explicacao': texto_explicacao # Adiciona a explicação ao dicionário
                })
...
def carregar_placar(modo):
    ...
    ## MUDANÇA: Usando resource_path para encontrar o arquivo de placar
...
# --- ROTAS DA APLICAÇÃO ---
# (As suas rotas estavam corretas, estou apenas garantindo que o arquivo fique completo e sem duplicatas)
...
def login():
    """ Verifica o código de acesso inserido pelo usuário. """
    codigo_inserido = request.form.get('codigo_acesso')

    if codigo_inserido == CODIGO_PISCICULTURA:
        # Se o código for o de piscicultura, guarda o nível de acesso na sessão
        session['access_level'] = 'piscicultura'
    elif codigo_inserido == CODIGO_TODOS:
        # Se o código for o geral, guarda o nível de acesso 'todos'
        session['access_level'] = 'todos'
    else:
        # Se o código for inválido, não faz nada (ou poderíamos mostrar uma mensagem de erro)
        pass
...
def responder(id_pergunta):
    ...
    # É AQUI que a decisão do modo de jogo acontece
    modo_de_jogo = session.get('modo_de_jogo', 'classico')
...
def revisao(id_pergunta):
    ...
    # Monta o dicionário de dados para enviar para a página
    dados_revisao = {
        'pergunta': questao_original['pergunta'],
        ...
        # LINHA CORRIGIDA: Agora estamos pegando e enviando a explicação
